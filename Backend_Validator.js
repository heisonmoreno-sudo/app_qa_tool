        ...
        funciones.forEach((func) => {
            if (typeof this[func] === "function") {
                ...
                resultado.advertencias.push(
                    modulo + ": Funcion " + func + " no encontrada"
                );
            }
        });
    ...
    try {
        ...
        if (configSheet) {
            ...
            resultado.errores.push(
                "Hoja Config NO encontrada - Sistema no funcionara"
            );
        }
        ...
        if (bugsSheet) {
            ...
            resultado.advertencias.push(
                "Hoja Bugs no encontrada - Funcionalidad limitada"
            );
        }

        const allSheets = ss.getSheets();
        const hojasCase = allSheets.filter(
            (s) => s.getName() !== "Config" && s.getName() !== "Bugs"
        );

        if (hojasCase.length > 0) {
            resultado.info.push(
                hojasCase.length +
                    " hoja(s) de casos encontradas: " +
                    hojasCase.map((s) => s.getName()).join(", ")
            );
        } else {
            resultado.advertencias.push(
                "No se encontraron hojas de casos de prueba"
            );
        }
    } catch (error) {
        resultado.advertencias.push(
            "No se puede validar Sheet (puede estar ejecutandose desde editor): " +
                error.message
        );
    }
}
...
function validarEstructuraFrontend(resultado) {
    ...
    try {
        ...
        resultado.errores.push(
            "Frontend_Index.html NO encontrado o tiene errores"
        );
    }
    ...
    componentesEsperados.forEach((comp) => {
        try {
            ...
            resultado.advertencias.push(
                "Componente no encontrado: " + comp + ".html"
            );
        }
    });
}
